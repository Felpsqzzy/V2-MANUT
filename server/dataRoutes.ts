...
async function getReadMode(req: Request, cfg: TableConfig): Promise<'global'|'self'|'none'> { if(await hasPermission(req,cfg.readPermission))return'global'; if(cfg.selfColumn&&await hasPermission(req,cfg.selfReadPermission))return'self'; return'none'; }
async function getWriteMode(req: Request, cfg: TableConfig): Promise<'global'|'self'|'none'> { if(await hasPermission(req,cfg.writePermission))return'global'; if(cfg.selfColumn&&await hasPermission(req,cfg.selfWritePermission))return'self'; return'none'; }
...
router.get('/data/:table', async (req,res,next)=>{try{const cfg=configs[req.params.table];if(!cfg)return res.status(404).json({error:'Recurso não disponível.'});const mode=await getReadMode(req,cfg);if(mode==='none')return res.status(403).json({error:'Acesso não autorizado.'});const filters:string[]=[];const params:unknown[]=[];for(const [key,value] of Object.entries(req.query)){if(!key.startsWith('eq_'))continue;const column=key.slice(3);if(!allowedFilterColumns.has(column))continue;params.push(normalizeValue(value));filters.push(`${column} = $${params.length}`)}if(mode==='self'&&cfg.selfColumn){params.push(req.authUserId);filters.push(`${cfg.selfColumn} = $${params.length}`)}let sql=`select ${cfg.select} from ${cfg.source}`;if(filters.length)sql+=` where ${filters.join(' and ')}`;const order=cleanOrder(req.query.order);const dir=String(req.query.direction||'asc').toLowerCase()==='desc'?'desc':'asc';if(order)sql+=` order by ${order} ${dir}`;const limit=Math.min(Math.max(Number(req.query.limit)||100,1),500);params.push(limit);sql+=` limit $${params.length}`;const r=await query(sql,params);res.json({data:r.rows})}catch(e){next(e)}});

router.post('/data/:table', async (req,res,next)=>{try{const cfg=configs[req.params.table];if(!cfg||!cfg.insert.size)return res.status(403).json({error:'Inserção não disponível neste recurso.'});const mode=await getWriteMode(req,cfg);if(mode==='none')return res.status(403).json({error:'Acesso não autorizado.'});const input=req.body&&typeof req.body==='object'?{...req.body}:{};if(mode==='self'&&cfg.selfColumn&&cfg.insert.has(cfg.selfColumn))input[cfg.selfColumn]=req.authUserId;const keys=Object.keys(input).filter(k=>cfg.insert.has(k));if(!keys.length)return res.status(400).json({error:'Nenhum campo permitido.'});const values=keys.map(k=>normalizeValue(input[k]));const placeholders=keys.map((_,i)=>`$${i+1}`).join(',');const r=await query(`insert into ${cfg.source} (${keys.join(',')}) values (${placeholders}) returning ${cfg.select}`,values);res.status(201).json({data:r.rows})}catch(e){next(e)}});

router.patch('/data/:table/:id', async (req,res,next)=>{try{const cfg=configs[req.params.table];if(!cfg||!cfg.update.size)return res.status(403).json({error:'Atualização não disponível neste recurso.'});const mode=await getWriteMode(req,cfg);if(mode==='none')return res.status(403).json({error:'Acesso não autorizado.'});const input=req.body&&typeof req.body==='object'?req.body:{};const keys=Object.keys(input).filter(k=>cfg.update.has(k));if(!keys.length)return res.status(400).json({error:'Nenhum campo permitido.'});const values=keys.map(k=>normalizeValue(input[k]));values.push(req.params.id);let where=`id=$${values.length}`;if(mode==='self'&&cfg.selfColumn){values.push(req.authUserId);where+=` and ${cfg.selfColumn}=$${values.length}`}const sql=`update ${cfg.source} set ${keys.map((k,i)=>`${k}=$${i+1}`).join(',')} where ${where} returning ${cfg.select}`;const r=await query(sql,values);if(!r.rowCount)return res.status(404).json({error:'Registro não encontrado.'});res.json({data:r.rows[0]})}catch(e){next(e)}});

router.post('/rpc/:name', async (req,res,next)=>{try{if(!req.authUserId)return res.status(401).json({error:'Sessão não autenticada.'});const name=req.params.name;let data:null|Record<string,unknown>=null;
  if(name==='get_my_access_context'){const r=await query('select get_user_access_context($1) data',[req.authUserId]);data=r.rows[0]?.data||null;}
  else if(name==='get_access_admin_data'){if(!(await hasPermission(req,'users.view')))return res.status(403).json({error:'Acesso não autorizado.'});const profiles=await query('select * from v_active_profiles order by name');const roles=await query('select id,code,name,description,active from roles where active order by name');const permissions=await query('select id,code,name,resource,action from permissions order by resource,name');const rp=await query('select r.code role_code,p.code permission_code from role_permissions x join roles r on r.id=x.role_id join permissions p on p.id=x.permission_id');for(const role of roles.rows){role.permissions=rp.rows.filter(x=>x.role_code===role.code).map(x=>x.permission_code)}data={profiles:profiles.rows,roles:roles.rows,permissions:permissions.rows};}
  else if(name==='admin_set_user_access'){const userId=String(req.body?.p_user_id||req.body?.user_id||'');const role=String(req.body?.p_role_code||req.body?.role_code||'');const active=req.body?.p_active??req.body?.active??true;if(!userId||!role)return res.status(400).json({error:'Usuário e perfil são obrigatórios.'});const r=await query('select admin_set_user_access($1,$2,$3,$4) data',[req.authUserId,userId,role,Boolean(active)]);data=r.rows[0]?.data||null;}
  else if(name==='has_permission'){const permission=String(req.body?.p_permission_code||req.body?.permission_code||'');data={allowed:Boolean((await query('select has_permission($1,$2) allowed',[req.authUserId,permission])).rows[0]?.allowed)};}
  else return res.status(404).json({error:'Operação não disponível.'});
  res.json({data});
}catch(e){next(e)}});
