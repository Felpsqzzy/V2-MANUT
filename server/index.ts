...
app.get('/api/meters', requireAuth, async (req, res) => {
...
app.post('/api/meters', requireAuth, requirePermission('meters.manage'), async (req, res) => {
  const payload = {
...
app.patch('/api/meters/:id', requireAuth, requirePermission('meters.manage'), async (req, res) => {
  const payload = {
...
app.post('/api/readings', requireAuth, requirePermission('readings.create'), async (req, res) => {
...
app.patch(
  ...
  async (req, res) => {
    ...
    if (!roleCode) return res.status(400).json({ error: 'role_code é obrigatório.' });
...
app.post(
  ...
  upload.single('file'),
  async (req, res) => {
    if (!req.file) return res.status(400).json({ error: 'Nenhum arquivo válido foi enviado.' });
    if (!serviceSupabase) {
...
app.use((err: unknown, _req: Request, res: Response, _next: NextFunction) => {
  ...
  if (message.includes('CORS')) return res.status(403).json({ error: message });
